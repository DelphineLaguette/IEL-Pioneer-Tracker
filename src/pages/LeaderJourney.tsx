...
function CheckInCard({ ci, color }: { ci: CheckIn; color: string }) {
  const principle = PRINCIPLES.find(p => p.id === ci.selectedPrinciple);
  const progressColors = { improved: 'text-green-600 bg-green-50', same: 'text-yellow-600 bg-yellow-50', declined: 'text-red-600 bg-red-50' };
    ...
    <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
      ...
      <div className="space-y-2 text-sm">
        ...
        {ci.supportNeeded && ci.typeOfSupportNeeded && (
          <div className="mt-2 p-2 bg-amber-50 border border-amber-100 rounded-lg text-xs">
            <span className="font-semibold text-amber-700">Support requested: </span>
            <span className="text-amber-800">{ci.typeOfSupportNeeded}</span>
          </div>
        )}
      </div>
  ...
  if (!leader) {
    return (
      <div className="text-center py-16">
        ...
        <Link to="/" className="mt-4 inline-block text-indigo-600 hover:underline">Back to dashboard</Link>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="h-2" style={{ backgroundColor: leader.color }} />
        <div className="p-6 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 mt-2 text-sm text-gray-500">
              {sp ? (
                <span className="text-green-600 font-medium">✓ Starting reflection complete</span>
              ) : (
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
              ...
              <PrincipleRadar startingPoint={sp} color={leader.color} />
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
              ...
              {sp.oneThingTeamShouldFeel && (
                <div className="mt-4 p-3 bg-indigo-50 rounded-lg">
                  <p className="text-xs font-medium text-indigo-700 mb-1">One thing I want my team to feel more of:</p>
                  <p className="text-sm text-indigo-900">{sp.oneThingTeamShouldFeel}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              {[
                { label: 'Strongest principle', value: sp.strongestPrinciple },
                { label: 'Main development area', value: sp.mainDevelopmentArea },
                { label: 'Why this matters', value: sp.whyDevelopmentAreaMatters },
                { label: 'Leadership intention', value: sp.leadershipIntention },
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-semibold text-gray-900 text-lg">30-Day Check-Ins</h2>
          {sp && (
            <Link
        ...
        {checkIns.length === 0 ? (
          ...
          <div className="space-y-4">
            {checkIns.map(ci => (
              <CheckInCard key={ci.id} ci={ci} color={leader.color} />
            ))}
          </div>
        )}
      </div>
